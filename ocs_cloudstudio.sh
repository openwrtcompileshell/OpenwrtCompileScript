...
SF="Script_File"
OW="Openwrt"
by="ITdesk&EldersJavas/EldersJavas"
...
#显示编译文件夹
ls_file() {
	LF=`ls workspace/$OW | grep -v $0  | grep -v Script_File`
...
ls_file_luci(){
	...
	if [[ -e workspace/$OW/$SF/tmp ]]; then
		echo ""
	else
		mkdir -p workspace/$OW/$SF/tmp	
	fi
	
	echo "$file" > workspace/$OW/$SF/tmp/you_file
	cd && cd workspace/$OW/$file/lede
}
...
download_package_luci() {
	cd workspace/$OW/$file/lede/package/Extra-plugin
	clear
	echo "	      -------------------------------------"
	echo "	      	    【 5.4额外的插件 】"
	echo ""
	...
	case "$download_num" in
		1)
		git clone https://gitee.com/OpenwrtChinaSource/luci-theme-argon.git
		;;
		2)
		git clone https://gitee.com/OpenwrtChinaSource/OpenAppFilter.git
		;;
		99)
...
download_package_customize() {	
	...
	echo "自定义下载插件"
	echo ""
	echo -e " $green例子：git clone https://gitee.com/OpenwrtChinaSource/OpenAppFilter.git   (此插件用于过滤应用)$white"
 	echo "--------------------------------------------------------------------------------"
	echo ""
...
source_RestoreFactory() {
	...
	make distclean
	ln -s workspace/$OW/$SF/dl  workspace/$OW/$file/lede/dl
...
source_update_No_git_pull() {
	source_branch=`cat "workspace/$OW/$SF/tmp/source_branch"`
	git fetch --all
...
description_if(){
	...
	if [[ ! -d "workspace/$OW/$SF/$OCS" ]]; then
		echo "开始创建主文件夹"
		mkdir -p workspace/$OW/$SF/dl
		mkdir -p workspace/$OW/$SF/My_config
		mkdir -p workspace/$OW/$SF/tmp
	fi

  	#清理一下之前的编译文件
   	rm -rf workspace/$OW/$SF/tmp/*

	#判断是否云编译
	workspace_home=`echo "workspace" | grep gitpod | wc -l`
	if [[ "$workspace_home" == "1" ]]; then
		...
		if [[ "$openwrt_shfile_path" == "0" ]]; then
			echo "export shfile=workspace/Openwrt/Script_File/OpenwrtCompileScript" | sudo tee -a /etc/profile
			echo -e "$green添加openwrt脚本变量成功,以后无论在那个目录输入 cd \$shfile 都可以进到脚本目录$white"
			#clear
		elif [[ "$openwrt_script_path" == "0" ]]; then
			echo "export openwrt=workspace/Openwrt/Script_File/OpenwrtCompileScript/openwrt.sh" | sudo tee -a /etc/profile
			#clear
			echo "-----------------------------------------------------------------------"
			echo ""
	...
	if [[ -e workspace/$OW/$SF/$OCS ]]; then
		echo "存在"
	else 
		cd workspace/$OW/$SF/
                git clone https://gitee.com/OpenwrtChinaSource/OpenwrtCompileScript.git
		cd 
		rm -rf `pwd`/$OCS
		cd workspace/$OW/$SF/$OCS
		bash openwrt.sh
	...
	if [[ -e workspace/$OW/$SF/description ]]; then
		self_test
		main_interface
	else
		clear
		...
		read -p "请输入密码:" ps
			if [[ $ps == $by ]]; then
				description >> workspace/$OW/$SF/description && clear && self_test && main_interface
			else
		...
		if [[ -e /etc/apt/sources.list.back ]]; then
			...
			read -p "是否替换软件源然后进行编译（1.yes，2.no）："  win10_select
				...
				1)
					...
					sudo cp workspace/$OW/$SF/$OCS/ubuntu18.4_sources.list /etc/apt/sources.list
					sudo apt-get update
...
create_file() {
	...
	if [[ -e workspace/$OW/$file ]]; then
		...
		echo "开始创建文件夹"
			mkdir workspace/$OW/$file
			cd workspace/$OW/$file  && clear
			echo "$file" > workspace/$OW/$SF/tmp/you_file
...
source_download_openwrt() {
		...
		read  -p "请输入你要下载的源代码:" Download_source_openwrt
			case "$Download_source_openwrt" in
				1)
				git clone https://gitee.com/OpenwrtChinaSource/openwrt-coolsnowwolf.git 
				;;
				2)
				git clone https://gitee.com/OpenwrtChinaSource/lede-coolsnowwolf.git 
				;;
				3)
				git clone https://gitee.com/OpenwrtChinaSource/openwrt-Lienol.git 
				;;
				4)
				git clone -b lede-17.01 https://gitee.com/OpenwrtChinaSource/openwrt.git
				;;
				5)
				git clone -b openwrt-18.06 https://gitee.com/OpenwrtChinaSource/openwrt.git 
				;;
				6)
				git clone -b openwrt-19.07 https://gitee.com/OpenwrtChinaSource/openwrt.git
				;;
				7)
				git clone  https://gitee.com/OpenwrtChinaSource/openwrt.git
				;;
				0)
				exit
				;;
	...
	clear
		...
		read  -p "请输入你要下载的源代码:" Download_source_pandorabox_sdk
			case "$Download_source_pandorabox_sdk" in
				...
				rm -rf lede/dl
				ln -s workspace/$OW/$SF/dl  workspace/$OW/$file/lede/dl
				wget --no-check-certificate https://raw.githubusercontent.com/coolsnowwolf/lede/master/feeds.conf.default -O workspace/$OW/$file/lede/feeds.conf.default
				source_lean_package
				cd workspace/$OW/$file/lede	
				source_Soft_link			
				update_feeds
...
source_download_if() {
		if [[ -e workspace/$OW/$file/lede ]]; then
			cd workspace/$OW/$file/lede
			source_if
			source_Soft_link
			update_feeds
...
source_if() {
		...
		if [[ `git remote -v | grep -o https://gitee.com/OpenwrtChinaSource/openwrt.git | wc -l` == "2" ]]; then
			echo "openwrt" > workspace/$OW/$SF/tmp/source_type
			if [[ $source_git_branch == "lede-17.01" ]]; then
				echo "lede-17.01" > workspace/$OW/$SF/tmp/source_branch
			elif [[ $source_git_branch == "openwrt-18.06" ]]; then
				echo "openwrt-18.06" > workspace/$OW/$SF/tmp/source_branch
			elif [[ $source_git_branch == "openwrt-19.07" ]]; then
				echo "openwrt-19.07" > workspace/$OW/$SF/tmp/source_branch
			elif [[ $source_git_branch == "master" ]]; then
				echo "master" > workspace/$OW/$SF/tmp/source_branch
			fi
		elif [[ `git remote -v | grep -o https://gitee.com/OpenwrtChinaSource/lede-coolsnowwolf.git | wc -l` == "2" ]]; then
			echo "lean" > workspace/$OW/$SF/tmp/source_type
		...
		elif [[ `git remote -v | grep -o https://gitee.com/OpenwrtChinaSource/openwrt-Lienol.git | wc -l` == "2" ]]; then
			echo "lienol" > workspace/$OW/$SF/tmp/source_type
			if [[ $source_git_branch == "my-19.07-full" ]]; then
				echo "my-19.07-full" > workspace/$OW/$SF/tmp/source_branch
			fi

		else
			echo -e  "检查到你的源码是：$red未知源码$white"
			echo -e  "是否继续运行脚本！！！运行请回车，不运行请终止脚本"
			echo "unknown" > workspace/$OW/$SF/tmp/source_type
			read a
		fi 
}
...
source_Soft_link() {
		...
		if [[ -e workspace/$OW/$SF/description ]]; then
			...
			description >> workspace/$OW/$SF/description
		fi

		#2
		if [[ -e workspace/$OW/$file/lede/dl ]]; then
			echo ""
		else
			ln -s  workspace/$OW/$SF/dl workspace/$OW/$file/lede/dl
		fi
		...
		#3
		if [[ -e workspace/$OW/$file/lede/My_config ]]; then
			echo ""
		else
			ln -s  workspace/$OW/$SF/My_config workspace/$OW/$file/lede/My_config
		fi

		#4
		if [[ -e workspace/$OW/$file/lede/openwrt.sh ]]; then
			echo ""
		else
			ln -s  workspace/$OW/$SF/$OCS/openwrt.sh workspace/$OW/$file/lede/openwrt.sh
		fi
...
source_openwrt() {
		clear
		source_type=`cat "workspace/$OW/$SF/tmp/source_type"`
...
source_openwrt_Setting() {
		...
		if [[ -e workspace/$OW/$file/lede/include/target.mk_back ]]; then
			echo ""
		else
			cp workspace/$OW/$file/lede/include/target.mk  workspace/$OW/$file/lede/include/target.mk_back		
		fi			

		ITdesk&EldersJavas_default_packages="DEFAULT_PACKAGES:=base-files libc libgcc busybox dropbear mtd uci opkg netifd fstools uclient-fetch logd urandom-seed urngd block-mount coremark kmod-nf-nathelper kmod-nf-nathelper-extra kmod-ipt-raw wget libustream-openssl ca-certificates default-settings luci luci-proto-relay   luci-app-sqm  luci-app-adbyby-plus luci-app-autoreboot luci-app-filetransfer luci-app-vsftpd luci-app-ssr-plus luci-app-arpbind luci-app-vlmcsd luci-app-wol luci-app-ramfree luci-app-sfe luci-app-flowoffload luci-app-nlbwmon luci-app-accesscontrol  luci-app-ttyd luci-app-watchcat luci-app-wifischedule luci-app-netdata  luci-app-frpc ddns-scripts_aliyun ddns-scripts_dnspod"
		...
		if [[ "$(grep -o "urngd" include/target.mk_back )" == "urngd" ]]; then
			...
			sed -i "s/DEFAULT_PACKAGES:=base-files libc libgcc busybox dropbear mtd uci opkg netifd fstools uclient-fetch logd urandom-seed urngd/$ITdesk&EldersJavas_default_packages/g"  include/target.mk
		else
			#17.1 and 18.6 (PACKAGES)
			sed -i "s/DEFAULT_PACKAGES:=base-files libc libgcc busybox dropbear mtd uci opkg netifd fstools uclient-fetch logd/$ITdesk&EldersJavas_default_packages/g"  include/target.mk
		fi
		...
		if [[ $(grep -o "upx" tools/Makefile | wc -l)  == "1" ]]; then
			...
			svn checkout https://github.com/coolsnowwolf/lede/trunk/tools/upx  workspace/$OW/$file/lede/tools/upx
			svn checkout https://github.com/coolsnowwolf/lede/trunk/tools/ucl  workspace/$OW/$file/lede/tools/ucl
...
source_openwrt_Setting_18() {
	...
	sed -i 's/default "5"/default "0"/g' workspace/$OW/$file/lede/config/Config-images.in

	#去掉IPV6(by:左右）
	sed -i 's/+IPV6:luci-proto-ipv6 //g' workspace/$OW/$file/lede/feeds/luci/collections/luci/Makefile

	#修改exfat支持(by:左右）
	sed -i 's/+kmod-nls-base @BUILD_PATENTED/+kmod-nls-base/g' workspace/$OW/$file/lede/feeds/packages/kernel/exfat-nofuse/Makefile
...
source_lean() {
	...
	if [[ "$source_type" == "lean" ]]; then
		...
		if [[ "$indexif" == "Local Weather" ]]; then
			...
			cp workspace/$OW/$SF/$OCS/Warehouse/index_Weather/index.htm feeds/luci/modules/luci-mod-admin-full/luasrc/view/admin_status/index.htm
		fi
		...
		if [[ "$x86indexif" == "Local Weather" ]]; then
			...
			cp workspace/$OW/$SF/$OCS/Warehouse/index_Weather/x86_index.htm package/lean/autocore/files/index.htm
		fi
		...
		if [[ -e package/other-plugins/luci-app-serverchan ]]; then
			...
			git clone https://gitee.com/OpenwrtChinaSource/luci-app-serverchan.git package/other-plugins/luci-app-serverchan
		fi
...
source_lean_package() {
	...
	svn checkout https://github.com/coolsnowwolf/lede/trunk/package/lean  workspace/$OW/$file/lede/package/lean
	if [[ $? -eq 0 ]]; then
...
source_lienol() {
	...
	if [[ "$source_type" == "lienol" ]]; then
		...
		if [[ -e workspace/$OW/$file/lede/package/lean/luci-app-ttyd ]]; then
			echo ""		
		else
			svn checkout https://github.com/coolsnowwolf/lede/trunk/package/lean/luci-app-ttyd workspace/$OW/$file/lede/package/lean/luci-app-ttyd
		fi
...
make_compile_firmware() {
	...
	#by：BoomLee  ITdesk&EldersJavas
}

if_wo() {
	if [[ $? -eq 0 ]]; then
		#复制编译好的固件过去
		workspace_if=`echo workspace | grep workspace | wc -l `
		if [[ "$workspace_if" == "1" ]]; then
			da=`date +%Y%m%d`
			HOME=`echo "$THEIA_WORKSPACE_ROOT"`
			source_type=`cat workspace/$OW/$SF/tmp/source_type`
			you_file=`cat workspace/$OW/$SF/tmp/you_file`
			...
			cd && cd workspace
			\cp -rf workspace/$OW/$you_file/lede/bin/targets/  workspace/bin/$da-$source_type
			echo -e "本次编译完成的固件已经copy到$green workspace/bin/$da-$source_type $white"
		fi
	else
		echo -e "$red>> 固件编译失败，请查询上面报错代码$white"
		make_continue_to_compile
...
n1_builder() {
	if [[ -e workspace/$OW/$SF/n1 ]]; then
		echo ""
	else
		mkdir workspace/$OW/$SF/n1
		n1_builder
	fi

	builder_patch="workspace/$OW/$SF/n1/PHICOMM-N1-OpenWRT-Image-Builder"
	if [[ -e bin/targets/armvirt/64/[$(date +%Y%m%d)]-openwrt-armvirt-64-default-rootfs.tar.gz ]]; then
		...
		if [[ -e $builder_patch ]]; then
			...
			git clone https://gitee.com/OpenwrtChinaSource/PHICOMM-N1-OpenWRT-Image-Builder.git $builder_patch
		fi
